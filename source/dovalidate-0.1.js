  ...
  Error : {
    ...
    writeMsg : function(obj) {
      if (typeof obj.err == 'undefined') {
        ...
        ((obj.parentNode).getElementsByTagName('label'))[0].appendChild(msgD);
        obj.err = true;
      }
    },
  ...
  Validate : {
    all : function(els) {
      ...
      for (ids in els) {
        ...
        var rules = els[ids];
        for (ruleid in rules) {
          if (!SWAPJS.doValidator.Validators[rules[ruleid]](elObj.value)) {
    ...
    inits : function(id, els) {
      ...
      for (ids in els) {
        ...
        if (elObj.val[0] == 'empty') {
          var manD = SWAPJS.doValidator.DOM.newElement('small', ' (Required)',{});
          SWAPJS.doValidator.DOM.addClass(manD, 'mandatory');
          ((elObj.parentNode).getElementsByTagName('label'))[0].appendChild(manD);
        }

        elObj.onblur = function() {
          var rules = this.val;
          for (ruleid in rules) {
            if (!SWAPJS.doValidator.Validators[rules[ruleid]](this.value)) {
